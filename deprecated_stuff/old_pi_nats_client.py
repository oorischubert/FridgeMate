...
PUBLISH_DELAY_SECONDS = 0.1           # Target ~10 FPS (adjust as needed)
...
async def main_loop(nats_url, use_pi_cam):
    ...
    try:
        ...
        print(f"Starting frame publishing loop to subject: '{FRAMES_SUBJECT}'")
        while not shutdown_event.is_set():
            ...
            if frame is None:
                await asyncio.sleep(0.1) # Wait a bit if capture failed
                continue
            ...
            # Publish frame
            try:
                await nc.publish(FRAMES_SUBJECT, encoded_frame.tobytes())
                # Optional: print confirmation, can be noisy
                # print(f"Published frame ({len(encoded_frame)} bytes)")
            except ConnectionClosedError:
                print("ERROR: NATS connection closed unexpectedly. Trying to reconnect?")
                # NATS client handles reconnection automatically based on options
                await asyncio.sleep(1) # Wait before next attempt
            except TimeoutError:
                print("WARN: Timeout publishing frame to NATS.")
            except Exception as e:
                print(f"ERROR: Failed to publish frame: {e}")


            # Maintain target frame rate / Prevent busy-looping
            elapsed_time = time.monotonic() - start_time
            sleep_duration = max(0, PUBLISH_DELAY_SECONDS - elapsed_time)
